...
OBJS		+= $(patsubst %.c,%.o,$(wildcard $(COMMON_DIR)/*.c)) $(patsubst %.c,%.o,$(wildcard $(LIB_DIR)/*.c)) $(patsubst %.c,%.o,$(wildcard $(SRC_DIR)/*.c))
...
DEFS		+= -I$(OPENCM3_DIR)/include -I$(COMMON_DIR)/ -I$(LIB_DIR)/  -I$(SRC_DIR)/
LDFLAGS		+= -L$(OPENCM3_DIR)/lib   -L$(COMMON_DIR)/ -L$(LIB_DIR)/ -L$(SRC_DIR)/
LDLIBS		+= -l$(LIBNAME)
LDSCRIPT	?= $(TARGET).ld
else
...
%.bin: %.elf
	@#printf "  OBJCOPY $(*).bin\n"
	$(Q)$(OBJCOPY) -Obinary $(BIN_DIR)/$(TARGET).elf $(BIN_DIR)/$(TARGET).bin
...
%.elf %.map: $(OBJS) $(LDSCRIPT)
	@#printf "  LD      $(*).elf\n"
	@$(Q)$(LD) $(TGT_LDFLAGS) $(LDFLAGS) $(OBJS) $(LDLIBS) -o $(BIN_DIR)/$(TARGET).elf
	$(SIZE) $(BIN_DIR)/$(TARGET).elf

$(TARGET).elf: $(OBJS)
	@mkdir -p $(BIN_DIR)
	@$(LD) $(TGT_LDFLAGS) $(LDFLAGS) $(OBJS) $(OBJ_DIR) $(LDLIBS) -o $(BIN_DIR)/$(TARGET).elf
